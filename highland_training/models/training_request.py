...
class TrainingRequest(models.Model):
    ...
    def _onchange_course_selection_type(self):
        ...
        if self.course_selection_type == 'new':
            # Nếu tạo khóa mới, xóa danh sách nhân viên (sẽ gán ở khóa học)
    ...
    def action_approve(self):
        ...
        for course in self.course_ids:
            # Chuyển khóa học sang 'Cần lập kế hoạch'
            if course.state == 'draft':
                course.state = 'need_planning'
            
            # Xác định danh sách nhân viên cần ghi danh
            employees_to_enroll = self.env['hr.employee']
            
            if self.course_selection_type == 'existing':
                # Khóa học có sẵn: lấy nhân viên từ nhu cầu
                employees_to_enroll = self.employee_ids
                # Thêm nhân viên vào khóa học (không xóa nhân viên cũ)
                if self.employee_ids:
                    course.employee_ids = [(4, emp.id) for emp in self.employee_ids]
            else:
                # Khóa học mới: lấy nhân viên từ khóa học (đã gán thủ công)
                employees_to_enroll = course.employee_ids
            
            # Tự động ghi danh nhân viên vào khóa học
            for employee in employees_to_enroll:
                ...
                if not existing:
                    enrollment_obj.create({
                        'employee_id': employee.id,
                        'course_id': course.id,
                        'state': 'draft',  # Trạng thái nháp, chờ lập kế hoạch
                    })
